/* sketch.js — the "drawn by hand" engine.
 *
...
const Sketch = (() => {
  ...
  /* Catmull-Rom → cubic bézier: turns a point list into one smooth stroke */
  function smooth(points, close = false) {
    const p = points;
    if (p.length < 2) return "";
    const pt = (i) =>
      p[close ? (i + p.length) % p.length : Math.max(0, Math.min(p.length - 1, i))];
  ...
  function circlePath(cx, cy, r, seed = 1, roughness = 1) {
    const rand = rng(seed);
  ...
  function squarePath(cx, cy, size, seed = 1, roughness = 1) {
    const rand = rng(seed);
    const h = size / 2;
    const corners = [
      [cx - h, cy - h], [cx + h, cy - h],
  ...
  function rectPath(cx, cy, w, h, seed = 1, roughness = 1) {
    const rand = rng(seed);
    const hw = w / 2, hh = h / 2;
    const corners = [
  ...
  function linePath(x1, y1, x2, y2, seed = 1, roughness = 1) {
    const rand = rng(seed);
  ...
  function drawOn(pathEl, { duration = 650, nibGroup = null, color = "#2a2521" } = {}) {
    return new Promise((resolve) => {
      ...
      const tick = (now) => {
        ...
        if (t < 1) {
          ...
          pathEl.style.strokeDasharray = "none";
          if (nib) {
            nib.style.transition = "opacity .25s ease";
            nib.style.opacity = "0";
            setTimeout(() => nib.remove(), 280);
          }
          resolve();
        }
      };
  ...
  function startBoil(filterTurbulenceEl, { fps = 6, seeds = [1, 7, 13] } = {}) {
    if (prefersReduced || !filterTurbulenceEl) return () => {};
    let i = 0;
    const id = setInterval(() => {
      i = (i + 1) % seeds.length;
      filterTurbulenceEl.setAttribute("seed", seeds[i]);
    }, 1000 / fps);
    return () => clearInterval(id);
  }

  return {
    el, circlePath, squarePath, rectPath, linePath, drawOn, startBoil, smooth, prefersReduced,
  };
})();
