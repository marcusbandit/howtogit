...
const laptopTreeEl = need("laptoptree");
const laptopList = need("laptop-list");
const gMiniDesk = need("mini-desktop");
const gMiniDeskInner = need("mini-desktop-inner");
...
function useMachine(m) {
    ...
    for (const g of [gEdges, gNodes, gNib, gLabels]) {
        g.style.transition = "none";
        g.style.opacity = onDesktop ? "1" : "0";
    }
    for (const g of [gLapEdges, gLapNodes, gLapNib, gLapLabels]) {
        g.style.transition = "none";
        g.style.opacity = onDesktop ? "0" : "1";
    }
}
// ---- step actions ---------------------------------------------------
...
async function doPushFromLaptop() {
...
let miniDesktopTrunk = []; // frozen snapshot of the desktop trunk at switch
...
const laptopVisible = () => stepIndex >= stepIdx("clone");
...
function laptopFileState(file) {
    const a3 = stepIdx("add3"), c3 = stepIdx("commit3"), p3 = stepIdx("push3");
    if (file === EDIT_FILE && a3 >= 0) {
        if (stepIndex === a3)
            return "modified"; // edited, not staged
        if (c3 >= 0 && stepIndex === c3)
            return "staged"; // staged, not committed
        if (c3 >= 0 && stepIndex > c3 && (p3 < 0 || stepIndex <= p3))
            return "committed"; // committed, not pushed
    }
...
function renderLaptopTree() {
    ...
    if (laptopHasProject) {
        ...
        if (!lastLaptopProject)
            proj.classList.add("is-new");
...
function renderMiniDesktop(animate = false) {
    ...
    shownMiniIds.clear();
    miniDesktopTrunk.forEach((n) => shownMiniIds.add(n.id));
}
...
async function playMoveToLaptop() {
...
function boot() {
    ...
    needSel("#laptoptree .tree__title").prepend(laptopIcon());
