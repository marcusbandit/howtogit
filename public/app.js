...
let projectedDrawDone = Promise.resolve();
...
function drawRefs(animateBranch) {
...
let dismissDemoHandler = null;
function clearDismissDemo() {
...
function rejectStaging() {
    ...
    dismissDemoHandler = (e) => {
        if (e instanceof KeyboardEvent)
            e.preventDefault(); // swallow Enter/Esc so it doesn't also act
        clearDismissDemo();
...
form.addEventListener("submit", async (e) => {
    ...
    if (busy)
        return;
    if (awaitingDismiss)
        return; // the dismiss handler drives what happens next
