...
function renderFileTree(): void {
  ...
  const fileStates = new Map((snap?.files ?? []).map((f) => [f.name, f.state] as const));
...
function appendGitNode(ul: HTMLElement, node: RepoGitNode, idx: number): void {
  ...
  if (node.isDir) {
    ...
    if (kids.length) kids.forEach((c, i) => appendGitNode(sub, c, i));
...
async function refreshRepo(): Promise<void> {
  ...
  if (steps[stepIndex]?.collapseTree) { treeOpen.clear(); treeOpen.add("root"); }
  renderFileTree();
}
...
function openCurio(btn: HTMLButtonElement, ans: HTMLElement, points?: string): void {
  ...
  if (points === "dotgit") { gitOpenBeforePeek = treeOpen.has(".git"); setGitOpen(true); }
  if (points) {
