...
async function doPull(): Promise<void> {
  if (active !== desktop) applyMoveToDesktop();
  const m = desktop.model;
  const trunk = () => m.nodes.filter((n) => n.lane === 0).sort((a, b) => a.col - b.col);
  let parent: CommitNode | null = trunk()[trunk().length - 1] ?? null;
  for (let i = trunk().length; i < remoteHistory.length; i++) {
    ...
    if (parent) await drawConnector(parent, node, COLORS.main, node.id * 7 + 4);
    await drawNode(node, node.id * 13 + 6);
    m.nodes.push(node);
    m.head = node.id;
    if (m.branches.main) m.branches.main.tip = node.id;
    parent = node;
  }
  if (parent) {
  ...
  drawRefs();
  if (parent) caption("pulled from the remote", parent.x, parent.y + NODE_R + 32, 320);
}
...
async function doClone(): Promise<void> {
  if (active !== laptop) applyMoveToLaptop();   // ensure the laptop is active (also on a bare seek)
  const m = laptop.model;
  const mainBranch: Branch = { color: COLORS.main, shape: "circle", lane: 0, tip: null };
  m.branches = { main: mainBranch };
  m.headBranch = "main";
  laptop.nextId = 0;
  // mirror the remote's history down onto the laptop, drawn left to right
  let prev: CommitNode | null = null;
  for (let i = 0; i < remoteHistory.length; i++) {
    ...
    if (prev) await drawConnector(prev, node, COLORS.main, node.id * 7 + 4);
    await drawNode(node, node.id * 13 + 6);
    m.nodes.push(node);
    m.head = node.id;
  ...
  if (prev) caption("cloned from the remote", prev.x, prev.y + NODE_R + 32, 360);
}
...
async function playAutoCd(): Promise<void> {
  if (instant || S.prefersReduced) return;        // a seek just shows the entered project
  const gen = ++autoCdGen; const alive = () => gen === autoCdGen;
  busy = true;                                     // block the user while it types itself
  autoTyping = true;                               // render the auto text neutrally (see updateInk)
  cmd.value = ""; updateInk();
  await sleep(520); if (!alive()) { busy = false; autoTyping = false; return; }
  const text = "cd my-site/";
  for (let n = 1; n <= text.length; n++) {
    cmd.value = text.slice(0, n); updateInk();
    await sleep(62); if (!alive()) { busy = false; autoTyping = false; return; }
  }
  await sleep(480); if (!alive()) { busy = false; autoTyping = false; return; }
...
interface Part { t: string; tone: Tone; why: string; span?: number; }
interface Teach { goal: string; why: string; parts: Part[]; note?: string; }
...
function renderActivePart(force = false, instant = false): void {
  ...
  for (const el of Array.from(partsEl.querySelectorAll<HTMLElement>(".part:not(.part--out)"))) {
...
form.addEventListener("submit", async (e) => {
  ...
  if (steps[stepIndex - 1]?.key === "clone") void playAutoCd();
  // finishing the laptop push lands us at `pull`: move back to the desktop
  if (stepIndex === stepIdx("pull")) void playMoveToDesktop();
});
...
type FileSide = "local" | "remote" | "laptop";   // which tree a file lives in
...
function renderFileTree(): void {
  ...
  PROJECT.files.forEach((f, fi) => {
    ...
    if (st === "pushed") pushedNow.add(f);
    // the moment a file lands on the remote, give it a quick purple flash
    if (st === "pushed" && !pushedBefore.has(f)) li.classList.add("is-pushed-now");
...
function mkIcon(kind: string): SVGSVGElement {
  const svg = S.el("svg", { viewBox: "0 0 24 24", "aria-hidden": "true" }) as SVGSVGElement;
...
function fileLines(file: string, side: FileSide): { lines: string[]; changed: number[] } {
  if (file === EDIT_FILE) {
    const a3 = stepIdx("add3"), c3 = stepIdx("commit3");
    let hasEdit = false;
    if (side === "local") hasEdit = stepIndex >= stepIdx("add2");
    else if (side === "remote") hasEdit = stepIndex > stepIdx("push2");
    else hasEdit = a3 >= 0 && stepIndex >= a3;           // laptop: its own edit (M3)
    if (hasEdit) {
...
const RULES: Record<Lang, SxRule[]> = { html: HTML_RULES, css: CSS_RULES, js: JS_RULES };
...
function fileRowRect(file: string, side: FileSide): DOMRect | null {
  const listEl = side === "remote" ? remoteList : side === "laptop" ? laptopList : treeList;
  const li = listEl.querySelector<HTMLElement>(`li[data-file="${file}"]`);
...
function openFileViewer(file: string, side: FileSide, row: HTMLElement): void {
...
function closeFileViewer(): void {
  ...
  editSeqGen++;
  editorEl.style.pointerEvents = "none";
  if (S.prefersReduced) { editorEl.style.opacity = "0"; return; }
  editorEl.style.transition =
...
function cancelAutoEdit(): void {
  if (viewerFile != null) return;   // a held-open click view is closeFileViewer's job
  editSeqGen++;
  editorEl.style.transition = "none";
  editorEl.style.opacity = "0";
  editorEl.style.pointerEvents = "none";
...
async function playEditSequence(side: FileSide = "local"): Promise<void> {
  ...
  await sleep(GROW_MS + 320); if (!alive()) return;   // grow, then a beat to take it in
  ...
  for (let n = 1; n <= EDIT_NEW.length; n++) {
    ...
    await sleep(44); if (!alive()) return;
  }
  await sleep(650); if (!alive()) return;       // sit on the finished edit a moment
  ...
  await sleep(1600); if (!alive()) return;      // longer hold so the save registers
...
function wireFileViewer(): void {
  const onList = (listEl: HTMLElement, side: FileSide) => {
    listEl.addEventListener("click", (e) => {
      const li = (e.target as HTMLElement).closest("li[data-file]") as HTMLElement | null;
      const file = li?.dataset.file;
      if (!file || !li) return;
      if (viewerFile === file && viewerSide === side) closeFileViewer();
      else openFileViewer(file, side, li);
    });
  };
  ...
  document.addEventListener("click", (e) => {
    ...
    if (editorEl.contains(t) || treeEl.contains(t) || remoteTreeEl.contains(t) || laptopTreeEl.contains(t)) return;
    closeFileViewer();
  });
...
function renderRemoteGraph(allowAnim = true): void {
  ...
  if (firstShow) { label.style.transition = "none"; label.style.transform = `translateY(${rise}px)`; label.style.opacity = "0"; }
  // hide each new connector so it can draw in only after its node has landed
  for (const p of parts) {
    if (p.conn) { const L = p.conn.getTotalLength(); p.conn.style.strokeDasharray = String(L); p.conn.style.strokeDashoffset = String(L); }
  }
  ...
  requestAnimationFrame(() => {
    if (firstShow) {
      ...
      label.style.transform = "translateY(0)"; label.style.opacity = "1";
    }
    for (const p of parts) {
...
async function playMoveToLaptop(): Promise<void> {
  ...
  try {
    ...
    requestAnimationFrame(() => { if (!alive()) return; for (const g of deskLayers) { g.style.transition = "opacity .75s ease"; g.style.opacity = "0"; } });
    const blob = makeBlob();
    positionBlobOver(blob, deskRect);                  // form over the desktop
    blob.style.opacity = "0"; void blob.getBoundingClientRect();
    requestAnimationFrame(() => { if (!alive()) return; blob.style.transition = "opacity .5s ease"; blob.style.opacity = "1"; });
    ...
    if (alive()) busy = false;   // only the current move releases the lock
  }
}
...
function applyMoveToDesktop(): void {
...
async function playMoveToDesktop(): Promise<void> {
  ...
  try {
    ...
    for (const g of deskLayers) { g.style.transition = "none"; g.style.opacity = "0"; }
    for (const g of lapLayers) { g.style.transition = "none"; g.style.opacity = "1"; }
    void deskLayers[0].getBoundingClientRect();
    requestAnimationFrame(() => {
      if (!alive()) return;
      for (const g of deskLayers) { g.style.transition = "opacity .75s ease"; g.style.opacity = "1"; }
      for (const g of lapLayers) { g.style.transition = "opacity .75s ease"; g.style.opacity = "0"; }
    });
    const blob = makeBlob();
    ...
    await sleep(1150); if (!alive()) { blob.remove(); return; }
    await sleep(220);
    blob.style.transition = "opacity .45s ease"; blob.style.opacity = "0";
    await sleep(460); if (!alive()) { blob.remove(); return; }
    blob.remove();
  } finally {
    if (alive()) busy = false;
...
// Two tiers. A SECTION ("The basics 1") groups the tasks of one whole arc; a
// TASK ("Initialize repo") groups the commands that make it up. The section
// you're in is expanded into its task milestones; sections you've finished (or
// not reached yet) compact into a single labelled square, so the timeline stays
// short as more arcs are added.
interface TLTask { label: string; keys: string[] }
interface TLSection { name: string; tasks: TLTask[] }
const TIMELINE_SECTIONS: TLSection[] = [
...
const tlSections: TLSectionStop[] = [];
...
function buildTimeline(): void {
  ...
  TIMELINE_SECTIONS.forEach((section, sIdx) => {
    if (sIdx > 0) linkInto(timelineEl);   // connector between sections
...
async function seekTo(target: number): Promise<void> {
  ...
  if (target >= stepIdx("clone") && (pullIdx < 0 || target < pullIdx) && active === desktop) applyMoveToLaptop();
  if (pullIdx >= 0 && target >= pullIdx && active === laptop) applyMoveToDesktop();
