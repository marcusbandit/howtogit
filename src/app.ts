...
const cue = need<HTMLElement>("cue");
const ghost = need<HTMLElement>("ghost");
...
function setCue(html: string): void {
  cue.style.opacity = "0";
  setTimeout(() => {
    cue.innerHTML = html;
    cue.style.opacity = "";
...
function showGhost(text: string): void { ghost.textContent = text; ghost.classList.add("show"); }
function clearGhost(): void { ghost.classList.remove("show"); }
function nudge(): void {
