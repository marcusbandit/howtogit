...
import {
  replayTo, snapshot,
  type Snapshot as RepoSnapshot, type GitNode as RepoGitNode, type RepoCmd,
} from "./repo.js";
...
function buildFolder(name: string, path: string, open: boolean, cls: string, opts: { note?: string; isNew?: boolean; rootWrap?: boolean } = {}): { row: HTMLElement; sub: HTMLElement; wrap: HTMLElement } {
...
function renderTree(tree: TreeView, model: RepoModel): void {
  ...
  if (model.git) {
    ...
    if (model.git.length) model.git.forEach((n, i) => appendGitNode(tree, gitF.sub, n, i));
...
function appendGitNode(tree: TreeView, ul: HTMLElement, node: RepoGitNode, idx: number): void {
...
function localModel(): RepoModel {
  ...
  const states = new Map((snap?.files ?? []).map((f) => [f.name, f.state] as const));
  const files: FileRow[] = PROJECT.files.map((f) => {
    ...
    const flash: FileRow["flash"] = (f === EDIT_FILE && st === "modified" && !wasEditing) ? "edited"
...
function openGitFile(path: string, row: HTMLElement, side: "local" | "remote" = "local"): void {
...
function wireTree(tree: TreeView): void {
  tree.el.addEventListener("click", (e) => {
    ...
    if (folder) {
      ...
      if (path === ".git" && tree.side === "local" && openBtn?.dataset.points === "dotgit") gitOpenBeforePeek = willOpen;
      return;
    }
...
function remoteModel(): RepoModel | null {
  ...
  const files: FileRow[] = pushed
    ? PROJECT.files.map((f) => ({ name: f, file: f, state: "committed" as FileState, flash: lastRemotePushed ? undefined : "pushed" as const }))
    : [];
  return {
    root: PROJECT.root,
...
function renderRemoteTree(): void {
  ...
  if (!model) { remoteList.replaceChildren(); lastRemoteShown = false; lastRemotePushed = false; return; }
...
interface TreeView { el: HTMLElement; open: Set<string>; side: "local" | "remote"; }
const localTree: TreeView = { el: treeList, open: new Set(["root"]), side: "local" };
const remoteTree: TreeView = { el: remoteList, open: new Set(["root"]), side: "remote" };
...
async function refreshRepo(): Promise<void> {
  ...
  if (steps[stepIndex]?.collapseTree) { localTree.open.clear(); localTree.open.add("root"); }
  renderFileTree();
}
...
function setNodeOpen(tree: TreeView, path: string, open: boolean): void {
  if (open) tree.open.add(path); else tree.open.delete(path);
...
function openCurio(btn: HTMLButtonElement, ans: HTMLElement, points?: string): void {
  ...
  if (points === "dotgit") { gitOpenBeforePeek = localTree.open.has(".git"); setGitOpen(true); }
  if (points) {
