...
export function smooth(points: Point[], close = false): string {
  const p = points;
...
export function circlePath(cx: number, cy: number, r: number, seed = 1, roughness = 1): string {
  const rand = rng(seed);
...
export function squarePath(cx: number, cy: number, size: number, seed = 1, roughness = 1): string {
  const rand = rng(seed);
  const h = size / 2;
  const corners: Point[] = [
    [cx - h, cy - h], [cx + h, cy - h],
...
export function rectPath(cx: number, cy: number, w: number, h: number, seed = 1, roughness = 1): string {
  const rand = rng(seed);
  const hw = w / 2, hh = h / 2;
  const corners: Point[] = [
...
export function linePath(x1: number, y1: number, x2: number, y2: number, seed = 1, roughness = 1): string {
  const rand = rng(seed);
...
export function drawOn(pathEl: SVGPathElement, opts: DrawOnOptions = {}): Promise<void> {
...
export function startBoil(turbulence: Element | null, opts: BoilOptions = {}): () => void {
  const { fps = 6, seeds = [1, 7, 13] } = opts;
  if (prefersReduced || !turbulence) return () => {};
  let i = 0;
